-- Fix security definer view warning by recreating views without security_barrier
-- and then update the code to use the new functions instead

-- Recreate the views without security_barrier to fix the warning
CREATE OR REPLACE VIEW milk_tests_view AS
SELECT 
    mt.id,
    mt.user_id,
    mt.product_id,
...
WHERE mt.user_id = auth.uid()  -- Only show user's own data through the view
GROUP BY 
...
CREATE OR REPLACE VIEW milk_tests_aggregated_view AS
SELECT 
    mt.product_id,
    mt.rating,
    mt.price_quality_ratio,
